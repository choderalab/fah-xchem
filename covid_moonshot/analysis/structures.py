...
def slice_snapshot(snapshot, project, run):
    ...
    htf = np.load(
        f"/home/server/server2/projects/{project}/RUNS/RUN{run}/htf.npz",
        allow_pickle=True,
    )["arr_0"].tolist()
...
if __name__ == "__main__":
    ...
    project, run, clone, gen, frame, fragment_id = 13422, 0, 0, 0, 2, "x10789"
    ...
    # Write protein PDB
    sliced_snapshots["protein"].save("protein.pdb")

    # Write old and new complex PDBs
    for name in ["old_complex", "new_complex"]:
        sliced_snapshots[name].save(f"{name}.pdb")

    # Write ligand SDFs
    from openeye import oechem

    for name in ["old_ligand", "new_ligand"]:
        with oechem.oemolostream(f"{name}.sdf") as ofs:
