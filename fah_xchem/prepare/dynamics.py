...
class FAHProject(BaseModel):
    project: str = Field(
        ..., description="Folding@Home project code")
    ...
    def _setup_fah_run(self,
        ...
        # Create barostat
        barostat = openmm.MonteCarloBarostat(self.simulation_parameters.pressure, self.simulation_parameters.temperature)
        ...
        forcefields = [self.simulation_parameters.protein_forcefield,
        ...
        if oemol is not None:
            ...
            logging.info(f'Creating virtual bond between atoms {protein_atom_index} and {ligand_atom_index}')
        ...
        logging.info(f'Initial potential energy is {state.getPotentialEnergy() / unit.kilocalories_per_mole:.3f} kcal/mol')
        ...
        if oemol is not None:
            ligand_atom_indices = mdtop.select('(resname MOL) and (mass > 1)')  # ligand heavy atoms
        trajectory = md.Trajectory(np.zeros([self.simulation_parameters.iterations + 1, len(atom_indices), 3], np.float32), mdtop)
        ...
        for iteration in track(range(self.simulation_parameters.iterations), 'Equilibrating...'):
        ...
        if restrain_rmsd:
            ...
            for iteration in track(range(self.simulation_parameters.iterations), 'Equilibrating without RMSD restraint...'):
                integrator.step(self.simulation_parameters.nsteps_per_iteration)
        ...
        logging.info(f'Final potential energy is {state.getPotentialEnergy() / unit.kilocalories_per_mole:.3f} kcal/mol')
        ...
        with open(os.path.join(destination_path, 'equilibrated-solute.pdb'), 'wt') as f:
            ...
            mdtraj_trajectory = md.Trajectory([state.getPositions(asNumpy=True) / unit.nanometers], mdtraj_topology)
        ...
        with open(os.path.join(destination_path, 'core.xml'), 'wt') as f:
            f.write(f'<config>\n')
            f.write(f'  <numSteps>{self.simulation_parameters.nsteps_per_snapshot * self.simulation_parameters.nsnapshots_per_wu}</numSteps>\n')
            f.write(f'  <xtcFreq>{self.simulation_parameters.nsteps_per_snapshot}</xtcFreq>\n')
