...
class Receptors(BaseModel):
    ...
    def generate_receptors(self) -> None:
        ...
        if not self._options_consistent():
            oechem.OEThrow.Verbose(
                f"Options are inconsistent. Ignoring this configuration: \n{self}"
            )
            errfs.close()
            return
        ...
        design_unit = create_dyad(
            "His41(0) Cys145(0)", docking_sytem, design_unit, options
        )
        ...
        oechem.OEThrow.Verbose("\t updating design unit...")
        design_unit = create_dyad(
            "His41(+) Cys145(-1)", docking_sytem, design_unit, options
        )
        oechem.OEThrow.Verbose("\t updating docking system...")
    ...
    def _create_output_filenames(self) -> OutputPaths:
        ...
        outputs = OutputPaths(
            ...
            receptor_thiolate_gzipped=output.joinpath(
                f"{stem}-receptor-thiolate.oeb.gz"
            ),
            design_unit_gzipped=output.joinpath(f"{stem}-designunit.oeb.gz"),
            design_unit_thiolate_gzipped=output.joinpath(
                f"{stem}-designunit-thiolate.oeb.gz"
            ),
    ...
    def _options_consistent(self) -> bool:
        ...
        if au_is_dimeric(self.input) and (not self.create_dimer):
            ...
            flag = False
        if (
            (not au_is_dimeric(self.input))
            and (crystal_series(self.input) in ["N", "P"])
            and self.create_dimer
        ):
    ...
    def _clean_pdb(self) -> List[str]:
        ...
        if (crystal_series(self.input) == "x") and (
            not is_in_lines(pdbfile_lines, "REMARK 350")
        ):
...
def get_chain_labels_pdb(pdb_path: Path) -> List[str]:
    ...
    return list(
        set([x[CHAIN_PDB_INDEX].lower() for x in pdb_lines if x.startswith("ATOM")])
    )
...
def set_options() -> oespruce.OEMakeDesignUnitOptions:
    ...
    opts.GetSplitOptions().SetMinLigAtoms(
        MINIMUM_FRAGMENT_SIZE
    )  # minimum fragment size (in heavy atoms)
...
def prevent_flip(
    options: oespruce.OEMakeDesignUnitOptions, match_strings: List[str]
...
def make_docking_system(design_unit: oechem.OEDesignUnit) -> DockingSystem:
    ...
    system = DockingSystem(
        protein=protein, ligand=ligand, receptor=receptor, design_unit=design_unit
    )
    return system
...
def write_docking_system(
    docking_system: DockingSystem,
    filenames: OutputPaths,
    is_thiolate: Optional[bool] = False,
) -> None:
    if is_thiolate:
...
def get_atoms(
    molecule: oechem.OEGraphMol, match_string: str, atom_name: str
...
def bypass_atoms(
    match_strings: List[str], options: oechem.OEPlaceHydrogensOptions
) -> oechem.OEPlaceHydrogensOptions:
    pred = oechem.OEAtomMatchResidue(match_strings)
